...
export class GobernanteGuard implements CanActivate {
  ...
  canActivate(
    ...
    // Permitir acceso a admin y gobernante
    const allowedRoles = ['admin', 'gobernante'];
    ...
    if (hasAllowedRole) {
      ...
      console.warn('🚫 Acceso denegado por GobernanteGuard:', {
        allowedRoles,
        isAdmin: this.authorizationService.isAdmin(),
        isGobernante: this.authorizationService.isGobernante(),
      ...
      Swal.fire({
        ...
        text: 'Solo gobernantes y administradores pueden acceder a esta sección',
        icon: 'error',
